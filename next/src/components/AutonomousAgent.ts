...
import type { ModelSettings } from ".";
...
class AutonomousAgent {
  ...
  async getInitialTasks(): Promise<string[]> {
    if (this.shouldRunClientSide()) {
      ...
      //await testConnection(this.modelSettings);
      return await AgentService.startGoalAgent(this.modelSettings, this.language, this.goal);
    }
  ...
  async executeTask(task: string, analysis: Analysis): Promise<string> {
    // Run search server side since clients won't have access to the API key
  ...
  sendGoalMessage() {
    this.renderMessage({ type: MESSAGE_TYPE_GOAL, value: this.goal });
  }
